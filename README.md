...
│
├──  data/
│   ├── input/                            # JSON input files (tests & definitions)
│   │   ├── function_calling_tests.json   # Natural language prompts (11 test cases)
│   │   ── functions_definition.json     # Available function schemas (5 functions)
│   └── output/                           # Generated results — DO NOT commit to Git
│       └── function_calling_results.json # Output: prompt + fn_name + args per prompt
│
├──  llm_sdk/                           # Provided SDK — copied as-is, not modified
│   └── llm_sdk/
│       └── __init__.py                   # Small_LLM_Model class (Qwen/Qwen3-0.6B wrapper)
│
├──  src/                               # Main source package
│   ├── __init__.py                       # Package marker
│   ├── __main__.py                       # Entry point for 'uv run python -m src'
│   │                                     #   argparse: --input / --output flags
│   │                                     #   loads files, runs pipeline, writes output
│   │
│   ├── models.py                         # Pydantic classes for validation:
│   │                                     #   FunctionParameter, FunctionDefinition
│   │                                     #   FunctionCallTest, FunctionCallResult
│   │
│   ├── constrained_dec.py                # Core: constrained decoding engine
│   │                                     #   VocabularyMapper  (token_id -> string)
│   │                                     #   JSONStateMachine  (tracks parse state)
│   │                                     #   mask_invalid_tokens()
│   │                                     #   generate()  (token-by-token loop)
│   │
...
│
├──  .gitignore                         # Excludes __pycache__, output/, .mypy_cache, etc.
├──  Makefile                           # Mandatory rules: install / run / debug / clean / lint
